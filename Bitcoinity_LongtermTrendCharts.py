...
for i in range(0,lnr_dr,1):
    line1,=plt.plot([year_dr[i],year_dr[i]],[0,p_perc_change_dr[i]],'-',color='xkcd:sky blue',linewidth=0.75,label='Price change [-]')
for i in range(0,lnr_dr,1):
    line2,=plt.plot([year_dr[i],year_dr[i]],[0,macd_histo_dr[i]],'b-',linewidth=0.75,label='MACD histogram')
line3,=plt.plot(year_dr,macd_dr,'k-',linewidth=0.25,label='MACD')
line4,=plt.plot(year_dr,macd_ema_dr,'r-',linewidth=0.25,label='EMA(MACD,9)')
leg=plt.legend(handles=[line1,line2,line3,line4],fontsize=5,loc='lower right',bbox_to_anchor=(0.99,0.01),edgecolor='none',facecolor='white')
leg.get_frame().set_linewidth(0.5)
leg.get_frame().set_alpha(1.0)
os.makedirs("site", exist_ok=True)
